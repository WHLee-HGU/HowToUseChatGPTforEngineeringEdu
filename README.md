
# ChatGPT를 활용한 교육방법에 대한 Study 및 사례 모음집

> **"One way to deal with that is to ban iPhones. Another way to do it is to make the class interesting.”** - Noam Chomsky

> **“학생들이 수업 시간에 스마트폰을 보는 것을 막기 위해 스마트폰 사용을 금지하는 방법이 있고 다른 하나는 스마트폰을 볼 생각이 들지 않을 정도로 수업을 충분히 재미있게 만드는 방법이 있다.”** - Noam Chomsky

* Chomsky’s solution for a functioning educational system in the age of ChatGPT is an interesting educational program with motivated students.
* ChatGPT 시대의 제대로 작동하는 교육 시스템을 위한 Chomsky의 솔루션은 **동기 부여**가 된 학생들이 참여하는 **흥미로운** 교육 프로그램입니다.
* https://the-decoder.com/chatgpt-is-high-tech-plagiarism-for-lazy-learners-says-noam-chomsky/ (Feb 13, 2023)

## Intro
본 문서는 다음의 목적을 가지고 있다.
1. ChatGPT를 공학분야(대학) 교육에 어떻게 활용할 수 있을지에 대해 자료를 수집
2. 실제 2023-1학기 수업을 진행하며 겪은 노하우 공유
3. 이후 공학교육에 어떻게 활용할 수 있을지에 대해 논의

특정 의견의 경우는 괄호()로 표시해 두었음


## ChatGPT란?

ChatGPT에 대한 기본적인 내용 및 관련 자료 수집은 별도의 GitHub 웹사이트([링크)](https://github.com/WHLee-HGU/ChatGPTScrap)에서 정리 중입니다.

### [한눈에 이슈] 서울대 AI원장도 "불가능" 교수·학생들 '대혼란' (KBS 뉴스 모음,  Feb 25, 2023)
* https://youtu.be/1kRFqNUd7Rc
* 위 링크에는 아래 5가지 뉴스 들이 합쳐진 것. 일반인 수준에서 이해하기 좋을 것 같아 별도로 가져옴
* ‘챗GPT’가 치른 수능 성적은?…선풍적 인기 속 윤리 합의 시급 / 2023.02.04 뉴스라인 / 조정인 기자
* 구글, MS ‘챗GPT 대항마’ 발표…AI 대전 본격화 / 2023.02.08 뉴스12 / 조정인 기자
* ‘챗GPT’ 유료 100만 돌파…우리 기업도 상반기 초거대 AI 출시 / 2023.02.17 뉴스9 / 김유대 기자
* “살인바이러스 개발” AI 채팅 논란…대리작성에 학계도 비상 / 2023.02.17 뉴스9 / 조정인 기자
* “챗GPT 열풍을 기회로”…국내 반도체 산업 재도약할까 / 2023.02.20 뉴스7 / 신지수 기자

### "20초 만에…" 초긴장, 대학생들도 술렁인다 / SBS / 모아보는 뉴스 (SBS 뉴스 모음,  Feb 18, 2023)
* https://youtu.be/fBtnlO45hw8
* 위 링크에는 아래 3가지 뉴스 들을 하나의 영상으로 모아놓은 것
* 기자는 반나절, 챗GPT는 20초?…정말 팩트체크 됐을까 (ChatGPT와 윤리)
* '챗GPT' 시대 도래…"이젠 발달하는 뇌 부위가 달라질 것" (ChatGPT와 뇌과학)
* 막을 수도, 권할 수도…챗GPT 두고 대학가 '술렁' (대학가 '혼란'도)

### Students who spoke to the ABC last week (Jan 26, 2023) had a mixed view on ChatGPT
* https://www.abc.net.au/news/2023-01-26/chatgpt-sparks-cheating-ethical-concerns-in-schools-universities/101888440
* 'Helpful for initial draft': student guild '초기 드래프트에 도움이 됨'
* 'Tertiary experience' at risk 위험에 처한 '3차 경험'
* Embrace technology: headmaster
* Cheating concerns downplayed 부정 행위 우려 경시




## 일반적인 교육에서의 활용 방법 및 사례

### ChatGPT에게 직접 물어보았다. (Feb 25, 2023)

#### 질문: “How to use ChatGPT in Education?”

#### 응답 (국문번역은 아래에)

There are a variety of ways in which ChatGPT can be used in education, some of which include:

* **Personalized learning**: ChatGPT can be used to provide personalized learning experiences to students. It can answer their questions, provide them with feedback, and suggest resources that match their learning style and pace.
* **Homework and assignments help**: ChatGPT can be used to help students with their homework and assignments. It can provide explanations, examples, and tips to help them understand concepts and solve problems.
* **Study aid**: ChatGPT can be used as a study aid for students. It can answer questions related to their subjects and provide them with resources such as study guides, videos, and articles to help them prepare for exams.
* **anguage learning**: ChatGPT can be used to help students learn a new language. It can provide them with practice exercises, vocabulary lists, and pronunciation tips.
* **Virtual assistant**: ChatGPT can act as a virtual assistant for teachers. It can help them manage their schedules, answer questions related to their subject, and even help with grading.
* **Accessibility**: ChatGPT can help make education more accessible for students with disabilities. It can provide them with text-to-speech capabilities, answer their questions, and even provide feedback on their work.
* Overall, ChatGPT can be a valuable tool in education, providing students with personalized support and helping them to learn more effectively.

ChatGPT를 교육에 사용할 수 있는 다양한 방법이 있으며 그 중 일부는 다음과 같습니다.

* **맞춤형 학습**: ChatGPT를 사용하여 학생들에게 맞춤형 학습 경험을 제공할 수 있습니다. 질문에 답하고 피드백을 제공하며 학습 스타일과 속도에 맞는 리소스를 제안할 수 있습니다.
* **숙제 및 과제 지원**: ChatGPT를 사용하여 학생들의 숙제 및 과제를 도울 수 있습니다. 개념을 이해하고 문제를 해결하는 데 도움이 되는 설명, 예 및 팁을 제공할 수 있습니다.
* **학습 지원**: ChatGPT는 학생들을 위한 학습 지원으로 사용할 수 있습니다. 과목과 관련된 질문에 답하고 시험 준비에 도움이 되는 학습 가이드, 비디오 및 기사와 같은 리소스를 제공할 수 있습니다.
* **언어 학습**: ChatGPT를 사용하여 학생들이 새로운 언어를 배울 수 있습니다. 연습 문제, 어휘 목록 및 발음 팁을 제공할 수 있습니다.
* **가상 비서**: ChatGPT는 교사를 위한 가상 비서 역할을 할 수 있습니다. 일정을 관리하고, 과목과 관련된 질문에 답하고, 채점에 도움을 줄 수 있습니다.
* **접근성**: ChatGPT는 장애가 있는 학생들이 교육에 더 쉽게 접근할 수 있도록 도와줍니다. 텍스트 음성 변환 기능을 제공하고, 질문에 답하고, 작업에 대한 피드백을 제공할 수도 있습니다.

전반적으로 ChatGPT는 교육에서 가치 있는 도구가 될 수 있으며, 학생들에게 개인화된 지원을 제공하고 보다 효과적으로 학습하도록 돕습니다.

### OpenAI의 의견 (Feb 2022)

원글 제목: Educator considerations for ChatGPT [링크: https://platform.openai.com/docs/chatgpt-education](https://platform.openai.com/docs/chatgpt-education)

(아래는 해당 블로그의 내용 중 일부만 발췌, 가능하면 전문을 확인하는 것을 추천)

#### 간소화되고 개인화된 교육으으로 활용

예시

* 수업 계획 및 기타 활동을 위한 초안 작성 및 브레인스토밍
* 퀴즈 질문 또는 기타 연습 문제 설계에 도움
* 맞춤형 튜터링 도구 실험
* 선호도에 따라 자료 맞춤화(언어 단순화, 다양한 읽기 수준에 맞게 조정, 다양한 관심사에 맞는 맞춤형 활동 만들기)
* 글의 일부에 대한 문법적 또는 구조적 피드백 제공
* 쓰기 및 코딩과 같은 영역에서 기술 향상 활동에 사용(코드 디버깅, 쓰기 수정, 설명 요청)
* 비판 AI 생성 텍스트

위의 몇 가지 사항은 ChatGPT의 개인화 도구로서의 잠재력을 활용하지만, 학생 개인 정보 보호, 편향된 대우, 건강에 해로운 습관 개발을 포함하여 이러한 개인화와 관련된 위험도 있습니다. 학생이 직접적인 감독 없이 이러한 서비스를 제공하는 도구를 사용하기 전에 학생과 교육자는 아래에 설명된 도구의 제한 사항을 이해해야 합니다.

마찬가지로 교사가 과제를 작성하거나 학생 에세이에 의견을 제공하는 데 도움이 되는 모델을 얻는 데 성공했다고 보고했지만 ChatGPT는 그 자체로 평가 도구로 신뢰되어서는 안 됩니다. 오히려 교사는 입력과 출력을 모두 신중하게 검토하고 사용 정책에 설명된 대로 AI 시스템을 사용하거나 의존한 위치를 공개해야 합니다.

#### Truthfulness

(이 부분을 잘 이해하면 교육에 활용하기 좋을 것을오 생각됨)
(단순 구글 변역입니다.)

ChatGPT와 같은 도구는 종종 합리적으로 들리는 답변을 생성할 수 있지만 일관성 있게 또는 모든 도메인에서 정확하다고 신뢰할 수는 없습니다. 때때로 모델은 이치에 맞지 않거나 잘못된 주장을 제시할 것입니다. 다른 경우에는 출처 이름, 직접 인용, 인용 및 기타 세부 사항을 조작할 수 있습니다. 또한 일부 주제에서 모델은 진실을 왜곡할 수 있습니다. 예를 들어 답이 없을 때 답이 하나라고 주장하거나 두 가지 반대 주장의 상대적인 강도를 잘못 표현함으로써 모델이 진실을 왜곡할 수 있습니다. 이러한 이유로 학생들이 신뢰할 수 있는 외부 소스를 사용하여 정보의 신뢰성을 평가하는 방법을 아는 것이 중요합니다.

ChatGPT가 항상 정확한 답변을 제공하지 못하는 이유 중 하나는 훈련 데이터가 2021년에 중단된다는 것입니다. 이는 ChatGPT가 현재 이벤트, 추세 또는 해당 시점 이후에 발생한 모든 것을 인식하지 못한다는 것을 의미합니다. 최신 지식이나 정보가 필요한 질문이나 주제에 적절하게 응답할 수 없습니다. 예를 들어 현재 미국 대통령이 누구인지, 무슨 요일인지 모를 수 있습니다.

ChatGPT에는 외부 기능이 없으며 외부 소스에서 검색할 수 없습니다. 이것은 인터넷, 검색 엔진, 데이터베이스 또는 현재 채팅 외부의 기타 정보 소스에 액세스할 수 없음을 의미합니다. 사실을 확인하거나 참조를 제공하거나 계산 또는 번역을 수행할 수 없습니다. 가지고 있는 컨텍스트(사용자 입력 정보, 학습 데이터)를 기반으로만 응답을 생성할 수 있습니다. 웹 브라우징 기능 및 사실적 정확성 향상은 WebGPT의 블로그 게시물에서 자세히 알아볼 수 있는 공개 연구 영역입니다.

ChatGPT는 과학이나 인문학의 복잡한 문제에서 잘 수행되지 않을 수 있으며 수학적 및 계산 능력에 제한이 있습니다. 또한 ChatGPT는 공개, 온라인 담론 또는 훈련 데이터에 자주 나타나지 않는 주제에 대해서는 잘 수행되지 않을 수 있습니다. 모델이 자신 있고 합리적으로 들리는 답변을 제공하는 것처럼 보일 수 있지만 이러한 제한 사항을 염두에 두는 것이 중요합니다.

#### 유해한 콘텐츠, 편견 및 고정관념

(오히려 이런 부분을 이용해 토론 주제를 만들 수 있겠음)

ChatGPT는 때때로 미묘한 방식으로 유해한 편견과 고정관념을 영속시키는 콘텐츠를 생성할 수 있습니다. 여기에는 집단에 대한 편향적이거나 틀에 박힌 묘사를 생성하는 것이 포함되며, 이는 특히 그러한 편향을 가르치거나 배우거나 강화하는 맥락에서 해로울 수 있습니다. 이 모델은 일반적으로 서양의 관점과 사람들을 반영하는 콘텐츠 쪽으로 편향되어 있습니다. 이에 대한 한 가지 예는 모델이 영어에서 가장 잘 작동하고 유해한 콘텐츠를 방지하기 위해 취한 일부 조치가 영어로만 평가되었다는 것입니다. 모델의 대화 특성은 또한 모델과의 상호 작용 과정에서 사용자 편향 및 선호도를 도입하거나 강화할 가능성이 있습니다.

바람직하지 않은 콘텐츠의 생성을 제한하기 위한 조치를 취했지만 ChatGPT는 모든 청중에게 적합하지 않은 출력을 생성할 수 있으며 교육자는 어린이와 함께 또는 교실 맥락에서 사용하는 동안 이를 염두에 두어야 합니다.


### 유투브 채널 MyF의 사례 모음 및 의견 (Feb 11, 2023)

유투브 영상 "ChatGPT를 활용한 수업 방법 모음" [링크: https://youtu.be/EI-wZctFWKw](https://youtu.be/EI-wZctFWKw)

#### ChatGPT 사용을 금지한 학교들

* 호즈 퀸즐랜드, 학생들의 접근 금지 (학교에서 차단)
* 뉴욕시와 시애틀을 포함한 일부 공립학교에서 네트워크 차단
* ChatGPT 작성글을 탐지하는 프로그램 도입에 찬성

#### ChatGPT 탐지 프로그램 등장

* OpenAI에서 출시는 했으나 정확도가 26%임
* GPTZero라는 툴도 출시되어 있음
* 정확도가 아주 높지는 않은 것으로 평가됨

#### ChatGPT 사용을 막는 것이 옳은가?

촘스키 교수, “학생들이 수업 시간에 스마트폰을 보는 것을 막기 위해 스마트폰 사용을 금지하는 방법이 있고 다른 하나는 스마트폰을 볼 생각이 들지 않을 정도로 수업을 충분히 재미있게 만드는 방법이 있다.”

#### 교육에 ChatGPT를 활용하는 법

* 기존 숙제, 과제 방식 바꾸기
* 오픈북 과제 폐지
* 00에 대해 다섯 페이지를 작성하시오 과제 폐지
* 수업 중에 과제하기, 손으로 작성하기, 조별 과제, 구술 시험 등으로 전환 모색
* 거꾸로 교실 사례: 동영상으로 공부는 집에서, 과제는 오히려 수업시간 중에
* AI가 답하기 어려운 과제 내주기 or 수업하기
* 자신의 삶과 현재 사건에 대한 글 쓰게 하기
* 최근 발생한 사건에 대한 글쓰기, 토론 (ChatGPT는 2021년 이후 데이터는 잘 반영하지 못한다는 점 활용, 물론 ChatGPT가 업데이트 될 수 있음)
* 챗봇이 정보를 가지고 있지 않은 새롭고 희귀한 것들 가르치기 (생각보다 챗봇이 알고 있는 것이 많아 쉽지 않은 방법일듯)
* ChatGPT를 계산기처럼 사용 (기계와 인간의 지능을 Colaborate 하는 것)

*“ChatGPT”로 젠더와 정신건강을 주제로 2편의 19세기 단편 소설을 비교, 대조하는 에세이 개요 만들기*

*이후 노트북을 치우고 에세이를 손으로 완성하기*

*실제로 오레곤주의 한 고등학교 선생님이 시도한 방식이고, 학생들이 내용을 심화시켜 내용을 완성시키는 결과를 얻고, AI와의 협업을 아이들이 공부하는 효과*

* 보조 교사로 활용하기

*먼저 글을 쓰고, 문법 오류를 찾게 시키고, 글을 다시 어떻게 구성하면 좋을지 의견을 물음*

*코딩도 마찬가지로 활용 (교육 격차 해소에 도움이 될지?)*

* AI의 응답을 학생들이 평가하도록 함

*철학, 비평 등에 관한 AI 작성문을 학생들이 직접 평가*

*인간의 생각만 이야기하는 것이 아니라 로봇의 생각을 이야기하는 것으로 확장*

* 토론 스파링 파트너 역할
*“동성애 결혼을 합법으로 만들 이유들로 나를 설득해라.”
(사고를 확장할 수 있는 계기, AI의 사고를 이해하는데 활용, AI와 함께 살아갈 것이라면 이런 교육도 중요)

#### ChatGPT에 대한 교육자 고려사항

OpenAI 블로그 글 소개이므로, 본 문서의 해당 내용 참고 ([링크](https://github.com/WHLee-HGU/HowToUseChatGPTforEngineeringEdu/blob/main/README.md#openai%EC%9D%98-%EC%9D%98%EA%B2%AC-feb-2022))

#### ChatGPT 등장으로 인한 교육의 변화

* From [시키는 사람]과 [수행하는 사람] To [시키는 사람]과 [수항해는 기계/AI]
* 교육의 전환: 앞기, 계산, 기존 틀에 맞추는 능력에서 기획, 지시, 비판, 조합, 구성하는 능력을 필요로 하게 되는 상황
* 과목의 취지에 따라서 어느 분야에서는 권장, 어느 분야에서는 금지
* 출처표기의 명확성을 강조
* 기초교육 선행 없이 무작정 활용하는 것은 위험하다는 의견 소개

### 유투브 채널 MyF의 사례 모음 및 의견 추 (Mar 1, 2023)

https://youtu.be/duRcF9dom5Q?t=107

#### 교사들에 미칠 영향
* 초거대 AI 대체 가능한 부분: 과거 지식 전달, 부가설명
* 초거대 AI 대체 불가능한 부분: 새로운 수업 구성, 학생들이 모르는 부분 파악, 육아 및 보육 역할, 사회성 형성



### 인공지능이 정리한 지식을, 창조적 아이디어로 확장하도록 동기부여할 수 있도록 (Feb 18, 2023)
* 공존을 위한 전환을 모색해야 할 때
* "답을 찾는 교육이었다면, 이제는 질문을 찾는 교육으로 전환해야 함" (전창배 사단법인 국제인공지능&윤리협회 이사장)
* https://youtu.be/fBtnlO45hw8?t=312

### 인공지능의 도움으로 다시 바둑에서 사람이 인공지능을 이긴 사례 (기사, Feb 19, 2023)
* 기사제목: “이사람 인간 맞아? 수상한데”...인공지능과 바둑 15전14승
* https://m.mk.co.kr/news/it/10650403?fbclid=IwAR302feKY5tKRD8F-kvU4MgiEj0XuAtp9ixOQAqjklNkoAuu-iQiOiSxzIs
* 미국 아마추어 2위 켈린 펠린, 현장 컴퓨터 도움 없이 15전 14승, 비결은 AI가 가르쳐준 AI 약점 공략, “넓고 크게 둬 산만하게 한 것 주효”
* (교육 관련 내용은 아니지만, 충분히 인사이트를 얻을 수 있는 내용이라 여기에 정리함)


### YouTube 채널 Joma Tech의 의견, "The future of software engineers with ChatGPT" (Feb 13, 2023)
* 아래 링크 영상 ("A recap of ChatGPT | tech news")의 한 챕터
* https://youtu.be/WVct5y3hBEg?t=756
* (ChatGPT로 요약 후 구글번역한 것입니다.)
* 비디오에서 발표자는 소프트웨어 엔지니어가 작업을 지원하는 ChatGPT 및 GitHub Copilot과 같은 AI 도구에 위협을 느껴서는 안 된다고 제안합니다. 대신 이러한 도구를 작업 흐름에 통합하여 소프트웨어 엔지니어링의 지루한 측면을 제거하고 더 어려운 작업에 집중해야 합니다. 발표자는 엔지니어가 자신의 기술과 AI 도구의 기능을 결합하여 공생 관계를 만들어 사이보그가 되어야 한다고 제안합니다.
* 특히 발표자는 GitHub Copilot이 새로운 언어 또는 API 라이브러리로 코딩하는 데 앞장서서 소프트웨어 엔지니어를 도울 수 있다고 말합니다. Copilot에 문제를 설명함으로써 엔지니어는 중요한 출발점을 확보하고 어려운 문제를 보다 쉽게 해결할 수 있습니다. Copilot은 또한 논리적 문제에 도움을 줄 수 있으며 구조화된 코드와 좋은 설명이 제공될 때 솔루션을 올바르게 얻는 경우가 많습니다.
* GitHub Copilot의 다음 대형 제품 릴리스를 통해 문제 또는 PR(Pull Request) 설명을 실제 완전한 코드 커밋으로 전환할 수 있을 것으로 예측합니다. 이는 소프트웨어 엔지니어링 프로세스를 더욱 간소화하여 엔지니어가 더 중요한 작업에 집중하고 AI 도구가 작업의 더 일상적인 측면을 처리할 수 있도록 합니다.


### 네이처에서 언급한 내용 중 교육에 적용해볼 법한 관련 내용 발췌 (Feb 2, 2023)
* 관련 내용은 다음의 링크에서 요약해두었음 ([링크](https://github.com/WHLee-HGU/ChatGPTScrap#%EB%84%A4%EC%9D%B4%EC%B2%98%EC%97%90%EC%84%9C-%EC%9E%91%EC%84%B1%ED%95%9C-%EC%97%B0%EA%B5%AC%EB%A5%BC-%EC%9C%84%ED%95%9C-5%EA%B0%80%EC%A7%80-%EC%9A%B0%EC%84%A0-%EC%88%9C%EC%9C%84-feb-2-2023))
* "전문가 중심의 사실 확인 및 검증 프로세스가 필수적"
* "AI 챗봇 탐지기를 만들어 쓸데없는 경쟁에 가담하기보다는 연구 커뮤니티와 게시자가 무결성, 투명성 및 정직성을 가지고 LLM을 사용하는 방법을 찾아야 함"
* "기술을 현명하게 사용하는 방법을 알 수 있도록 LLM의 타당성과 신뢰성을 조사하고 발전시키는 것이 중요"
* "교육자들은 학부생들과 그것의 사용과 윤리에 대해 이야기해야 합니다."
* "외부 규칙이 없는 이 초기 단계에서는 책임 있는 그룹 리더와 교사가 정직, 무결성 및 투명성을 가지고 사용 방법을 결정하고 일부 참여 규칙에 동의하는 것이 중요합니다."
* "연구에 기여하는 모든 사람은 ChatGPT로 생성되었는지 여부에 관계없이 자신의 작업에 대해 책임을 져야 한다는 점을 상기해야 합니다."
* "모든 작성자는 자신의 텍스트, 결과, 데이터, 코드 및 참조를 신중하게 사실 확인해야 할 책임이 있습니다."

### 영어교육 유투버의 설명에서 발췌
* 챗GPT (ChatGPT) 영작 활용법 | 복잡하고 긴문장도 3초 안에 영작완료 (Feb 28, 2023)
* https://youtu.be/tiEOA4u56sI
* 아래 발췌문에서 (영어)를 다른 학문으로 바꾸어도 그대로 적용 가능할듯
* "내가 왜 (영어)를 배워야 하는지 그리고 내 (영어) 수준이 내 (영어) 상태가 어느 정도인지를 한번 체크해보고 인지하는 시간이 꼭 필요합니다. 그리고 더욱 중요한 건 내가 부족한 부분을 느낄 때 그냥 넘어가는 것이 아니라 그 부분을 학습하고 개선시키는 습관을 기르는 것이 중요합니다. 그 습관만 기르면 우리의 (언어) 실력은 시간과 비례해서 엄청난 향상이 일어날 거예요. 반대로 그 습관이 없다면 꾸준히 (언어) 공부를 해도 성장 속도가 비교적 더딜 수밖에 없습니다."
* "좋은 기능이 있어도 누군가는 그냥 호기심 정도로만 보고 넘어갈 것이고 누군가는 직접 써보고 느끼면서 계속해서 활용해 갈 것이에요. 이게 진짜 무서운 게 그렇게 되면 기술이 발전할수록 우리의 지식 빈부격차는 계속해서 커져 나갈 겁니다. 가끔 이런 기술 나오면 단순히 '어, 그럼 이제 영어 공부 안 해도 되겠네'라고 물어보시는 분들이 계시는데, 제가 지금 한국어로 적고 있지만 보통 저는 ChatGPT를 영어로 사용해요. 왜냐면 영어로 사용할 때 훨씬 더 고급 정보를 얻을 수 있고, 이 시스템도 영어에 최적화되어 있거든요."



## 공학 교육에서 활용 사례

### 적극적 활용 사례 (긍정적/부정적 평가는 추후에 업데이트)

**국내**

#### ChatGPT 활용 오픈 북 시험 볼 것 (Feb 18, 2023)
* 이경전 경희대 빅데이터응용학과 교수
* "오히려 학생들이 인공지능을 더 개발하고 개선하려는 그런 노력도 가지게 될 것"
* https://youtu.be/fBtnlO45hw8?t=327

#### '자료구조' 과목에서는 학생들에게 챗GPT와 협업해서 코딩 과제를 하도록 유도할 계획 (Feb 23, 2023)
* 문병로 서울대 컴퓨터공학 교수
* "산성이 높아지면서 절약한 시간만큼 수준을 높일 것이다. 전문가의 수련 결과는 질문을 적절하게 던져 원하는 답을 얻을 수 있는 능력과 깊게 연동될 것이다. 코딩 능력의 스펙트럼이 넓어지고, 초중급 인력의 수요는 감소하고, 고급 인력의 수요는 늘 것"
* https://www.mk.co.kr/news/contributors/10657265 (매일경제)

#### '알고리즘 분석' 과목에서 자신이 짠 코드와 ChatGPT의 결과를 비교하게 시킬 것 (Feb 27, 2023)
* 이원형 한동대 전산및전자공학부 교수
* 추가로 ChatGPT가 만든 문서를 나누어주고, 수업시간에서만 다룬 내용과 다른 점과 빠진 점 등을 찾아보게 하는 과제도 구상 중
* (제가 본 GitHub 작성자라서 제 계획도 넣어보았습니다. ㅎㅎ)

#### 'AI프로젝트입문' 과목에서 ChatGPT가 제대로 답변을 못하는 사례를 찾고 왜 그런지 분석하도록 시킬 것 (Feb 27, 2023)
* 이원형 한동대 전산및전자공학부 교수
* (제가 본 GitHub 작성자라서 제 계획도 넣어보았습니다. ㅎㅎ)

#### “챗GPT 활용 없는 과제는 감점"
* 정승익 서울사이버대 겸임교수
* ‘메타버스 현황과 미래’ 과목 (Feb 22, 2023)
* http://www.kookje.co.kr/news2011/asp/newsbody.asp?code=0300&key=20230222.99099006327
* 인터뷰 (Feb 28, 2023)
* https://youtu.be/nqnr45rHNcU
* 보고서 A4절반 분량은 직접 자료조사, 나머지는 ChatGPT로 작업
* "질문이 정확하고 구체적일 수록 더 많은 컨텐츠와 정보 제공하기 때문에, 공부를 많이 할 수록 좋은 질문을 할 수 있다는 철학
* '오픈북' 시험과 비슷?
* "아직은 답변에서 ChatGPT로 작성한 것이라는 패턴이 보인다."
* "오히려 교수까지 속일 정도면 의도한 답을 만들어 낸 것이기 때문에 학점을 주겠다."
* "정보를 크로스 체크, 팩트체크 하는 게 그 수강생의 능력일 것"
* "기술의 지속가능성을 과도기를 통해 확인해보게 될 것"



**해외**

#### My class required AI. Here's what I've learned so far. (블로그, Feb 17, 2023)
* 수업에서 적극적 활용한 사례로 학생들을 잘 훈련시켜 ChatGPT를 사용하게 하면 긍정적인 면이 크다고 평가
* https://oneusefulthing.substack.com/p/my-class-required-ai-heres-what-ive?fbclid=IwAR04j-pO1I3ee-GvQc_nSzffBWvVwm8nWLW6EJfWH1XrMc38VYousHLdlIE
* "it has been very successful, but there are some lessons to be learned"
* 수업 시간에 policy와 주의점을 미리 설명하였음
* 주의사항을 대략 정리하면, 1) 입력을 대충하면 결과도 별로다. 2) 결과를 다 믿지는 마라. 3) AI 사용했다는 것을 밝히고 입력을 뭐라고 썼는지 첨부할 것. 4) 아무렇게나 가져다 쓰지 마라
* "훈련이 안된 상태에서는, 모두가 AI를 잘못이용했다." (<- 여기에 대해서는 본문을 정독할 필요가 있겠다.)
* "학생들은 AI의 정확도, 신뢰도, 편향 이슈와 우려에 대해 생각보다 빨리 이해했다."




### 금지 사례 또는 부작용

**국내**

#### ChatGPT로 영문에세이 과제 대필, 전원 0점 (기사, Feb 9, 2023)
* 국내 수도권의 한 국제학교 사례
* [기사링크](https://www.donga.com/news/Society/article/all/20230209/117801590/1)



**해외**

#### ChatGPT **banned** from New York City public schools’ devices and networks (기사, Jan 6, 2023)
* https://www.nbcnews.com/tech/tech-news/new-york-city-public-schools-ban-chatgpt-devices-networks-rcna64446
* New York City's Department of Education announced a ban on the wildly popular chatbot ChatGPT — which some have warned could inspire more student cheating — from its schools’ devices and networks.
* * "While the tool may be able to provide quick and easy answers to questions, it does not build critical-thinking and problem-solving skills, which are essential for academic and lifelong success," Lyle said in a email statement.
* "이 도구는 질문에 대한 빠르고 쉬운 답변을 제공할 수 있지만 학업 및 평생 성공에 필수적인 비판적 사고 및 문제 해결 기술을 구축하지는 않습니다."라고 Lyle은 이메일 성명에서 말했습니다.

#### ChatGPT **banned** in WA public schools in time for start of school year (기사, Jan 30, 2023)
* https://www.abc.net.au/news/2023-01-30/chatgpt-to-be-banned-from-wa-public-schools-amid-cheating-fears/101905616
* WA is the latest Australian jurisdiction to ban ChatGPT, following New South Wales, Queensland, and Tasmania.

#### ChatGPT banned in some schools, but many experts say it can improve education (기사, Feb 7, 2023)
* https://cosmosmagazine.com/technology/chatgpt-ban-in-schools/
* 지난 주 서호주와 빅토리아주 공립학교는 학생들이 학교에 있는 동안 ChatGPT 사용을 전면 금지하기 위해 뉴사우스웨일즈, 퀸즐랜드, 태즈매니아에 합류했습니다. 이것은 ChatGPT가 미국, 프랑스 및 인도의 학군에서 금지되었기 때문에 나온 것입니다.



## 향후 공학교육에서 ChatGPT(또는 유사 AI 서비스) 활용 방안

### 의견 정리

#### 대학교 이전 교육에는 도입을 최소화 하자는 의견이 많이 보임
#### 대학교 교육에서 금지하는 것은 불가능함
#### 대학교 학생들 정도의 수준이면 AI 서비스의 한계점과 문제점을 스스로 충분히 인지할 수 있을 것
* 이번 학기를 지나며 확인이될 듯
#### 질문과 대화를 통해 자신의 생각을 더 깊게 정리할 수 있는 기회로 삼을 수 있음
* 학습에 효과적인 질문 방식들에 대한 정리 필요
#### 답을 찾던 학습에서 질문을 찾는 학습으로의 전환 시기
